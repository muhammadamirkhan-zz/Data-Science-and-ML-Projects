...
def main():
    ...
    if classifier == 'Random Forest':
        st.sidebar.subheader('Model Hyperparameters')
        n_estimators = st.sidebar.number_input('No of estimators', 100, 5000, step=25, key='n_estimators')
        max_depth = st.sidebar.number_input('Max depth of tree', 1, 20, step=1, key='max_depth')
        bootstrap = st.sidebar.radio('Bootstrap sample values when building trees', ('True', 'False'), key='bootstrap')
        metrics = st.sidebar.multiselect('What metrics to plot ?',
                                         ('Confusion Matrix', 'ROC Curve', 'Precision-Recall Curve'))
        if st.sidebar.button('Classify', key='classify'):
